...
export default function LiveChartUpdater({ slug, chart, pollingMs = 30000 }: Props) {
  useEffect(() => {
    let timer: any
    let aborted = false
    ...
    async function fetchLatest() {
      try {
        const res = await fetch(`/api/markets/${encodeURIComponent(slug)}/latest`, { cache: 'no-store' })
        if (!res.ok) return
        ...
        if (chart === 'panel') {
          // Optionally update open/close patti placeholders when present
    ...
    fetchLatest()
    timer = setInterval(() => { if (!aborted) fetchLatest() }, pollingMs)

    return () => {
      aborted = true
      if (timer) clearInterval(timer)
    }
  }, [slug, chart, pollingMs])
