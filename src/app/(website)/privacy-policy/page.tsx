  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        const policyBlocks = allBlocks.filter((b) => String(b.key).toUpperCase().startsWith('PRIVACY_POLICY'))
        if (active) setBlocks(policyBlocks)
      } catch (e: any) {
        console.error('Privacy policy load error:', e)
        if (active) setError(e?.message || 'Failed to load privacy policy')
      } finally {
    ...
    <div className="max-w-4xl mx-auto px-3 sm:px-4 md:px-6 py-6">
      <Card className="bg-slate-900/60 border border-slate-700/50">
        <CardHeader>
          ...
          <CardDescription className="text-slate-300">Our commitment to your privacy</CardDescription>
        </CardHeader>
        <CardContent>
            ...
            <div className="prose prose-invert max-w-none text-sm leading-relaxed space-y-4">
              {blocks.length ? (
                ...
                <div className="text-slate-300 text-sm">
                  Admin can add privacy policy content under USER_CONTENT section with keys starting with PRIVACY_POLICY.
                </div>
              )}
            </div>
          )}
